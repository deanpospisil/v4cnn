# -*- coding: utf-8 -*-
"""
Created on Wed Mar  9 14:38:06 2016
...
rect = np.vstack((np.ones((rect_len, 1)), np.zeros((ashape.shape[0]-rect_len,1))))
shift = round(rect_len/2)
rect = rect[ [(x + shift)%len(rect) for x in range(len(rect))] ]
rect = rect / np.linalg.norm(rect)

l_ashape = np.real(np.fft.ifft( 
            np.fft.fft(ashape, axis=0) * np.fft.fft(rect, axis=0), axis=0))

l_ashape = np.real(np.fft.ifft( 
            np.fft.fft(ashape, axis=0) * rect, axis=0))
